//! Nscale Client
//!
//! Request transformation and response processing for Nscale API
...
impl NscaleClient {
    /// Transform ChatRequest to Nscale API format
    ///
    /// Nscale uses OpenAI-compatible format
    ...
    /// Transform Nscale response to standard ChatResponse
    pub fn transform_chat_response(response: Value) -> Result<ChatResponse, ProviderError> {
        ...
        if let Some(error) = response.get("error") {
            let error_message = error
                .get("message")
                .and_then(|m| m.as_str())
                .unwrap_or("Unknown error from Nscale API");
            ...
            return Err(match error_code {
                "invalid_api_key" | "authentication_failed" => {
                    ProviderError::authentication("nscale", error_message)
                }
                "rate_limit_exceeded" => ProviderError::rate_limit("nscale", None),
                "model_not_found" => ProviderError::model_not_found("nscale", error_message),
                _ => ProviderError::api_error("nscale", 400, error_message),
            });
        }
        ...
        let mut response_obj = response
            .as_object()
            .ok_or_else(|| {
                ProviderError::response_parsing("nscale", "Response is not an object".to_string())
            })?
            .clone();
        ...
        if !response_obj.contains_key("id") {
            ...
            response_obj.insert(
                "id".to_string(),
                Value::String(format!("chatcmpl-nscale-{}", timestamp)),
            );
        }
        ...
        if !response_obj.contains_key("model") {
            response_obj.insert(
                "model".to_string(),
                Value::String("meta-llama/Llama-3.1-8B-Instruct".to_string()),
            );
        }
    ...
    pub fn supported_models() -> Vec<ModelInfo> {
        get_nscale_registry().get_all_models()
    }
...
mod tests {
    ...
    fn test_transform_request() {
        ...
        let transformed = NscaleClient::transform_chat_request(request);
        assert_eq!(transformed["model"], "meta-llama/Llama-3.1-8B-Instruct");
    ...
    fn test_supported_models() {
        let models = NscaleClient::supported_models();
        assert!(models.len() >= 5); // At least 5 models should be supported

        let model_ids: Vec<String> = models.iter().map(|m| m.id.clone()).collect();
        assert!(model_ids.contains(&"meta-llama/Llama-3.1-8B-Instruct".to_string()));
    }

    #[test]
    fn test_supported_openai_params() {
        let params = NscaleClient::supported_openai_params();
