...
pub struct AnyscaleConfig {
    #[serde(flatten)]
    pub base: BaseConfig,
}

impl Default for AnyscaleConfig {
    fn default() -> Self {
        Self {
            base: BaseConfig {
                api_key: None,
...
impl AnyscaleConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("ANYSCALE_API_KEY")
            .map_err(|_| "ANYSCALE_API_KEY environment variable is required")?;
        Ok(Self::new(api_key))
    }
...
impl ProviderConfig for AnyscaleConfig {
    fn validate(&self) -> Result<(), String> {
        self.base.validate("anyscale")?;
        Ok(())
    }
