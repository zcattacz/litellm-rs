...
const MANUS_CAPABILITIES: &[ProviderCapability] = &[
...
impl ManusProvider {
    pub async fn new(config: ManusConfig) -> Result<Self, ProviderError> {
        config
            .validate()
    ...
    async fn execute_request(
        ...
        let response = self
            ...
            .await
            .map_err(|e| ProviderError::network("manus", e.to_string()))?;
        ...
        if !status.is_success() {
            ...
            return Err(match status.as_u16() {
                400 => ProviderError::invalid_request("manus", error_body.to_string()),
        ...
        serde_json::from_slice(&response_bytes).map_err(|e| {
            ProviderError::api_error("manus", 500, format!("Failed to parse response: {}", e))
        })
    }
}
...
impl LLMProvider for ManusProvider {
    ...
    async fn transform_request(
        ...
        serde_json::to_value(&request)
            .map_err(|e| ProviderError::invalid_request("manus", e.to_string()))
    }
    ...
    async fn chat_completion(
        ...
        debug!("Manus AI chat request: model={}", request.model);
    ...
    async fn calculate_cost(
        ...
        let model_info = get_model_info(model).ok_or_else(|| {
            ProviderError::model_not_found("manus", format!("Unknown model: {}", model))
        })?;
