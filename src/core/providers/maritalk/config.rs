...
pub struct MaritalkConfig {
    #[serde(flatten)]
    pub base: BaseConfig,
}

impl Default for MaritalkConfig {
    fn default() -> Self {
        Self {
            base: BaseConfig {
                api_key: None,
...
impl MaritalkConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("MARITALK_API_KEY")
            .map_err(|_| "MARITALK_API_KEY environment variable is required")?;
        Ok(Self::new(api_key))
    }
...
impl ProviderConfig for MaritalkConfig {
    fn validate(&self) -> Result<(), String> {
        self.base.validate("maritalk")?;
        Ok(())
    }
