...
fn test_nvidia_nim_error_display() {
    let err = NvidiaNimError::ApiError("API failure".to_string());
    assert_eq!(err.to_string(), "API error: API failure");

    let err = NvidiaNimError::AuthenticationError("Bad key".to_string());
    assert_eq!(err.to_string(), "Authentication failed: Bad key");

    let err = NvidiaNimError::RateLimitError("Too many requests".to_string());
    assert_eq!(err.to_string(), "Rate limit exceeded: Too many requests");

    let err = NvidiaNimError::InvalidRequestError("Missing field".to_string());
    assert_eq!(err.to_string(), "Invalid request: Missing field");

    let err = NvidiaNimError::ModelNotFoundError("unknown-model".to_string());
    assert_eq!(err.to_string(), "Model not found: unknown-model");

    let err = NvidiaNimError::ServiceUnavailableError("Down for maintenance".to_string());
    assert_eq!(err.to_string(), "Service unavailable: Down for maintenance");

    let err = NvidiaNimError::StreamingError("Connection lost".to_string());
    assert_eq!(err.to_string(), "Streaming error: Connection lost");

    let err = NvidiaNimError::ConfigurationError("Invalid config".to_string());
    assert_eq!(err.to_string(), "Configuration error: Invalid config");

    let err = NvidiaNimError::NetworkError("Timeout".to_string());
    assert_eq!(err.to_string(), "Network error: Timeout");

    let err = NvidiaNimError::UnknownError("Something went wrong".to_string());
...
fn test_nvidia_nim_error_type() {
    ...
    assert_eq!(NvidiaNimError::ApiError("".to_string()).error_type(), "api_error");
    assert_eq!(NvidiaNimError::AuthenticationError("".to_string()).error_type(), "authentication_error");
    assert_eq!(NvidiaNimError::RateLimitError("".to_string()).error_type(), "rate_limit_error");
    assert_eq!(NvidiaNimError::InvalidRequestError("".to_string()).error_type(), "invalid_request_error");
    assert_eq!(NvidiaNimError::ModelNotFoundError("".to_string()).error_type(), "model_not_found_error");
    assert_eq!(NvidiaNimError::ServiceUnavailableError("".to_string()).error_type(), "service_unavailable_error");
    assert_eq!(NvidiaNimError::StreamingError("".to_string()).error_type(), "streaming_error");
    assert_eq!(NvidiaNimError::ConfigurationError("".to_string()).error_type(), "configuration_error");
    assert_eq!(NvidiaNimError::NetworkError("".to_string()).error_type(), "network_error");
    assert_eq!(NvidiaNimError::UnknownError("".to_string()).error_type(), "unknown_error");
}
...
fn test_nvidia_nim_error_is_retryable() {
    ...
    assert!(!NvidiaNimError::ModelNotFoundError("".to_string()).is_retryable());
    assert!(!NvidiaNimError::StreamingError("".to_string()).is_retryable());
    assert!(!NvidiaNimError::ConfigurationError("".to_string()).is_retryable());
    assert!(!NvidiaNimError::UnknownError("".to_string()).is_retryable());
}
...
fn test_nvidia_nim_error_retry_delay() {
    ...
    assert_eq!(NvidiaNimError::RateLimitError("".to_string()).retry_delay(), Some(60));
    assert_eq!(NvidiaNimError::ServiceUnavailableError("".to_string()).retry_delay(), Some(5));
    assert_eq!(NvidiaNimError::NetworkError("".to_string()).retry_delay(), Some(2));
    assert_eq!(NvidiaNimError::ApiError("".to_string()).retry_delay(), None);
    assert_eq!(NvidiaNimError::AuthenticationError("".to_string()).retry_delay(), None);
}
...
fn test_nvidia_nim_error_http_status() {
    ...
    assert_eq!(NvidiaNimError::AuthenticationError("".to_string()).http_status(), 401);
    assert_eq!(NvidiaNimError::RateLimitError("".to_string()).http_status(), 429);
    assert_eq!(NvidiaNimError::InvalidRequestError("".to_string()).http_status(), 400);
    assert_eq!(NvidiaNimError::ModelNotFoundError("".to_string()).http_status(), 404);
    assert_eq!(NvidiaNimError::ServiceUnavailableError("".to_string()).http_status(), 503);
    assert_eq!(NvidiaNimError::ApiError("".to_string()).http_status(), 500);
}

#[test]
fn test_nvidia_nim_error_to_provider_error_conversion() {
    use crate::core::providers::unified_provider::ProviderError;

    let err: ProviderError = NvidiaNimError::AuthenticationError("bad key".to_string()).into();
    assert!(matches!(err, ProviderError::Authentication { .. }));

    let err: ProviderError = NvidiaNimError::RateLimitError("limit".to_string()).into();
    assert!(matches!(err, ProviderError::RateLimit { .. }));

    let err: ProviderError = NvidiaNimError::InvalidRequestError("bad req".to_string()).into();
    assert!(matches!(err, ProviderError::InvalidRequest { .. }));

    let err: ProviderError = NvidiaNimError::ModelNotFoundError("model".to_string()).into();
    assert!(matches!(err, ProviderError::ModelNotFound { .. }));

    let err: ProviderError = NvidiaNimError::ConfigurationError("config".to_string()).into();
    assert!(matches!(err, ProviderError::Configuration { .. }));

    let err: ProviderError = NvidiaNimError::NetworkError("network".to_string()).into();
...
fn test_nvidia_nim_error_mapper_http_errors() {
    ...
    let err = mapper.map_http_error(400, "Invalid parameters");
    ...
    let err = mapper.map_http_error(502, "Bad gateway");
    assert!(matches!(err, NvidiaNimError::ServiceUnavailableError(_)));

    let err = mapper.map_http_error(503, "Unavailable");
    assert!(matches!(err, NvidiaNimError::ServiceUnavailableError(_)));
