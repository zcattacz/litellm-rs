...
impl YiProvider {
    pub fn new(config: YiConfig) -> Result<Self, ProviderError> {
        config
            .validate()
            .map_err(|e| ProviderError::configuration("yi", e))?;

        let http_client = reqwest::Client::builder()
            .timeout(config.timeout())
            .build()
            .map_err(|e| {
                ProviderError::initialization("yi", format!("Failed to create HTTP client: {}", e))
            })?;
    ...
    pub async fn with_api_key(api_key: impl Into<String>) -> Result<Self, ProviderError> {
        let config = YiConfig::new(api_key);
        Self::new(config)
    }
...
impl LLMProvider for YiProvider {
    ...
    type ErrorMapper = super::error_mapper::YiErrorMapper;
    ...
    async fn transform_request(
        ...
        if let Some(temperature) = request.temperature {
            req["temperature"] = serde_json::to_value(temperature)
                .map_err(|e| ProviderError::serialization("yi", e.to_string()))?;
        }
    ...
    fn get_error_mapper(&self) -> Self::ErrorMapper {
        super::error_mapper::YiErrorMapper
    }
    ...
    async fn chat_completion(
        ...
        let response = req_builder
            ...
            .send()
            .await
            .map_err(|e| ProviderError::network("yi", e.to_string()))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(match status {
                401 => ProviderError::authentication("yi", error_text),
                429 => ProviderError::rate_limit("yi", None),
                404 => ProviderError::model_not_found("yi", request.model),
                _ => ProviderError::api_error("yi", status, error_text),
            });
        }
    ...
    async fn calculate_cost(
        ...
        let model_info = self
            ...
            .iter()
            .find(|m| m.id == model)
            .ok_or_else(|| ProviderError::model_not_found("yi", model.to_string()))?;
