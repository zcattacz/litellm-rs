...
impl YiConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("YI_API_KEY")
            .map_err(|_| "YI_API_KEY environment variable is required")?;
        Ok(Self::new(api_key))
    }
...
impl ProviderConfig for YiConfig {
    fn validate(&self) -> Result<(), String> {
        self.base.validate("yi")?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_config_validation() {
        let mut config = YiConfig::default();
