...
pub struct DeepLConfig {
    ...
    /// Whether to use DeepL Pro API (default: false for free API)
    #[serde(default)]
    pub use_pro: bool,
}
...
impl DeepLConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("DEEPL_API_KEY")
            .map_err(|_| "DEEPL_API_KEY environment variable is required")?;
        Ok(Self::new(api_key))
    }
...
impl ProviderConfig for DeepLConfig {
    fn validate(&self) -> Result<(), String> {
        self.base.validate("deepl")?;
        Ok(())
    }
