...
#[derive(Debug, Clone)]
pub struct EmpowerProvider {
    config: EmpowerConfig,
...
impl EmpowerProvider {
    ...
    pub fn new(config: EmpowerConfig) -> Result<Self, ProviderError> {
        config
            .validate()
            .map_err(|e| ProviderError::configuration("empower", e))?;

        let pool_manager = Arc::new(
            GlobalPoolManager::new()
                .map_err(|e| ProviderError::configuration("empower", e.to_string()))?,
        );
        let supported_models = EmpowerClient::supported_models();
...
#[async_trait]
impl LLMProvider for EmpowerProvider {
    type Config = EmpowerConfig;
    type Error = ProviderError;
    type ErrorMapper = EmpowerErrorMapper;
    ...
    fn get_supported_openai_params(&self, _model: &str) -> &'static [&'static str] {
        EmpowerClient::supported_openai_params()
    }
    ...
    async fn transform_request(
        ...
        Ok(EmpowerClient::transform_chat_request(request))
    }
    ...
    async fn transform_response(
        ...
        let response: ChatResponse = serde_json::from_slice(raw_response)
            .map_err(|e| ProviderError::response_parsing("empower", e.to_string()))?;
        Ok(response)
    }
    ...
    async fn chat_completion(
        ...
        let body = EmpowerClient::transform_chat_request(request.clone());
        ...
        let response_bytes = response
            .bytes()
            .await
            .map_err(|e| ProviderError::network("empower", e.to_string()))?;
    ...
    ) -> Result<Pin<Box<dyn Stream<Item = Result<ChatChunk, Self::Error>> + Send>>, Self::Error>
    {
        ...
        let mut body = EmpowerClient::transform_chat_request(request.clone());
        ...
        let api_key = self
            ...
            .ok_or_else(|| ProviderError::authentication("empower", "API key is required"))?;
        ...
        Ok(Box::pin(super::streaming::create_empower_stream(stream)))
    }
