...
impl BytezProvider {
    ...
    pub async fn with_api_key(api_key: impl Into<String>) -> Result<Self, ProviderError> {
        let config = BytezConfig::new(api_key);
        Self::new(config)
    }
    ...
    async fn transform_request(
        ...
        if let Some(temperature) = request.temperature {
            req["temperature"] = serde_json::to_value(temperature)
                .map_err(|e| ProviderError::serialization("bytez", e.to_string()))?;
        }
    ...
    async fn chat_completion(
        ...
        if !response.status().is_success() {
            ...
            return Err(match status {
                401 => ProviderError::authentication("bytez", error_text),
                429 => ProviderError::rate_limit("bytez", None),
                404 => ProviderError::model_not_found("bytez", request.model),
                _ => ProviderError::api_error("bytez", status, error_text),
            });
        }
