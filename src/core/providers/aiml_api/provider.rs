...
use super::config::AimlConfig;
use super::model_info;

#[derive(Debug, Clone)]
pub struct AimlProvider {
    config: AimlConfig,
...
impl AimlProvider {
    pub fn new(config: AimlConfig) -> Result<Self, ProviderError> {
        config
            .validate()
            .map_err(|e| ProviderError::configuration("aiml", e))?;
    ...
    pub async fn with_api_key(api_key: impl Into<String>) -> Result<Self, ProviderError> {
        let config = AimlConfig::new(api_key);
        Self::new(config)
    }
...
#[async_trait]
impl LLMProvider for AimlProvider {
    type Config = AimlConfig;
    type Error = ProviderError;
    type ErrorMapper = super::error_mapper::AimlErrorMapper;
    ...
    async fn transform_request(
        ...
        if let Some(temperature) = request.temperature {
            req["temperature"] = serde_json::to_value(temperature)
                .map_err(|e| ProviderError::serialization("aiml", e.to_string()))?;
        }
    ...
    fn get_error_mapper(&self) -> Self::ErrorMapper {
        super::error_mapper::AimlErrorMapper
    }
    ...
    async fn chat_completion(
        ...
        let response = req_builder
            ...
            .send()
            .await
            .map_err(|e| ProviderError::network("aiml", e.to_string()))?;
    ...
    async fn calculate_cost(
        ...
        let model_info = self
            ...
            .iter()
            .find(|m| m.id == model)
            .ok_or_else(|| ProviderError::model_not_found("aiml", model.to_string()))?;
