...
impl SiliconFlowProvider {
    pub fn new(config: SiliconFlowConfig) -> Result<Self, ProviderError> {
        config
            .validate()
            .map_err(|e| ProviderError::configuration("siliconflow", e))?;
    ...
    pub async fn with_api_key(api_key: impl Into<String>) -> Result<Self, ProviderError> {
        let config = SiliconFlowConfig::new(api_key);
        Self::new(config)
    }
...
impl LLMProvider for SiliconFlowProvider {
    type Config = SiliconFlowConfig;
    type Error = ProviderError;
    type ErrorMapper = super::error_mapper::SiliconFlowErrorMapper;
    ...
    async fn transform_request(
        ...
        if let Some(temperature) = request.temperature {
            req["temperature"] = serde_json::to_value(temperature)
                .map_err(|e| ProviderError::serialization("siliconflow", e.to_string()))?;
        }
    ...
    fn get_error_mapper(&self) -> Self::ErrorMapper {
        super::error_mapper::SiliconFlowErrorMapper
    }
    ...
    async fn chat_completion(
        ...
        let response = req_builder
            ...
            .send()
            .await
            .map_err(|e| ProviderError::network("siliconflow", e.to_string()))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(match status {
                401 => ProviderError::authentication("siliconflow", error_text),
                429 => ProviderError::rate_limit("siliconflow", None),
                404 => ProviderError::model_not_found("siliconflow", request.model),
                _ => ProviderError::api_error("siliconflow", status, error_text),
            });
        }
    ...
    async fn calculate_cost(
        ...
        let model_info = self
            ...
            .iter()
            .find(|m| m.id == model)
            .ok_or_else(|| ProviderError::model_not_found("siliconflow", model.to_string()))?;
