...
use super::config::PredibaseConfig;
...
const PROVIDER_NAME: &str = "predibase";

const PREDIBASE_CAPABILITIES: &[ProviderCapability] = &[
...
impl PredibaseProvider {
    pub async fn new(config: PredibaseConfig) -> Result<Self, ProviderError> {
        config
            .validate()
...
impl LLMProvider for PredibaseProvider {
    ...
    async fn chat_completion(
        ...
        let response = self
            .execute_request("/v1/chat/completions", request_json)
            .await?;
