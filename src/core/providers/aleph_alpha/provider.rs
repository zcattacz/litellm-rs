...
use super::config::AlephAlphaConfig;
use super::model_info;

#[derive(Debug, Clone)]
pub struct AlephAlphaProvider {
    config: AlephAlphaConfig,
    http_client: reqwest::Client,
    supported_models: Vec<ModelInfo>,
}

impl AlephAlphaProvider {
    pub fn new(config: AlephAlphaConfig) -> Result<Self, ProviderError> {
        config
            .validate()
            .map_err(|e| ProviderError::configuration("aleph_alpha", e))?;
    ...
    pub async fn with_api_key(api_key: impl Into<String>) -> Result<Self, ProviderError> {
        let config = AlephAlphaConfig::new(api_key);
        Self::new(config)
    }
...
impl LLMProvider for AlephAlphaProvider {
    type Config = AlephAlphaConfig;
    type Error = ProviderError;
    type ErrorMapper = super::error_mapper::AlephAlphaErrorMapper;
    ...
    async fn transform_request(
        ...
        if let Some(temperature) = request.temperature {
            req["temperature"] = serde_json::to_value(temperature)
                .map_err(|e| ProviderError::serialization("aleph_alpha", e.to_string()))?;
        }
    ...
    fn get_error_mapper(&self) -> Self::ErrorMapper {
        super::error_mapper::AlephAlphaErrorMapper
    }
    ...
    async fn chat_completion(
        ...
        let response = req_builder
            ...
            .send()
            .await
            .map_err(|e| ProviderError::network("aleph_alpha", e.to_string()))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(match status {
                401 => ProviderError::authentication("aleph_alpha", error_text),
                429 => ProviderError::rate_limit("aleph_alpha", None),
                404 => ProviderError::model_not_found("aleph_alpha", request.model),
                _ => ProviderError::api_error("aleph_alpha", status, error_text),
            });
        }
    ...
    async fn calculate_cost(
        ...
        let model_info = self
            ...
            .iter()
            .find(|m| m.id == model)
            .ok_or_else(|| ProviderError::model_not_found("aleph_alpha", model.to_string()))?;
