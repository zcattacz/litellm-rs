...
pub struct AlephAlphaConfig {
    #[serde(flatten)]
    pub base: BaseConfig,
}

impl Default for AlephAlphaConfig {
    fn default() -> Self {
        Self {
            base: BaseConfig {
                api_key: None,
...
impl AlephAlphaConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("ALEPH_ALPHA_API_KEY")
            .map_err(|_| "ALEPH_ALPHA_API_KEY environment variable is required")?;
        Ok(Self::new(api_key))
    }
...
impl ProviderConfig for AlephAlphaConfig {
    fn validate(&self) -> Result<(), String> {
        self.base.validate("aleph_alpha")?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_default_config() {
        let config = AlephAlphaConfig::default();
        assert_eq!(
    ...
    fn test_config_validation() {
        let mut config = AlephAlphaConfig::default();
