//! Cerebras Client
//!
//! Request transformation, response processing, and model definitions for Cerebras AI
...
impl CerebrasClient {
    /// Transform chat request to Cerebras format (OpenAI compatible)
    ...
    /// Transform Cerebras response to standard format
    pub fn transform_chat_response(response: Value) -> Result<ChatResponse, ProviderError> {
        ...
        if let Some(error) = response.get("error") {
            let error_message = error
                .get("message")
                .and_then(|m| m.as_str())
                .unwrap_or("Unknown error from Cerebras API");
            ...
            return Err(match error_code {
                "authentication_error" | "invalid_request_error" => {
                    ProviderError::authentication("cerebras", error_message)
                }
                "rate_limit_exceeded" => ProviderError::rate_limit("cerebras", None),
                _ => ProviderError::api_error("cerebras", 400, error_message),
            });
        }
        ...
        let mut response_obj = response
            .as_object()
            .ok_or_else(|| {
                ProviderError::response_parsing("cerebras", "Response is not an object".to_string())
            })?
            .clone();
        ...
        if !response_obj.contains_key("id") {
            ...
            response_obj.insert(
                "id".to_string(),
                Value::String(format!("chatcmpl-cerebras-{}", timestamp)),
            );
        }
        ...
        if !response_obj.contains_key("model") {
            response_obj.insert(
                "model".to_string(),
                Value::String("llama3.1-70b".to_string()),
            );
        }
    ...
    pub fn supported_models() -> Vec<ModelInfo> {
        get_cerebras_registry().get_all_models()
    }
...
mod tests {
    ...
    fn test_transform_request() {
        ...
        let transformed = CerebrasClient::transform_chat_request(request);
    ...
    fn test_supported_models() {
        let models = CerebrasClient::supported_models();
        assert!(models.len() >= 2);

        let model_ids: Vec<String> = models.iter().map(|m| m.id.clone()).collect();
        assert!(model_ids.contains(&"llama3.1-70b".to_string()));
        assert!(model_ids.contains(&"llama3.1-8b".to_string()));
    }

    #[test]
    fn test_supported_openai_params() {
        let params = CerebrasClient::supported_openai_params();
    ...
    fn test_transform_response_not_object() {
        ...
        let result = CerebrasClient::transform_chat_response(invalid_response);
        assert!(result.is_err());
    }
}
