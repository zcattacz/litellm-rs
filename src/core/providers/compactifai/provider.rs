...
use super::config::CompactifaiConfig;
use super::model_info;

#[derive(Debug, Clone)]
pub struct CompactifaiProvider {
    config: CompactifaiConfig,
    http_client: reqwest::Client,
    supported_models: Vec<ModelInfo>,
}

impl CompactifaiProvider {
    pub fn new(config: CompactifaiConfig) -> Result<Self, ProviderError> {
        config.validate().map_err(|e| {
            ProviderError::configuration("compactifai", e)
        })?;
        ...
        let http_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                ProviderError::initialization("compactifai", format!("Failed to create HTTP client: {}", e))
            })?;
    ...
    pub async fn with_api_key(api_key: impl Into<String>) -> Result<Self, ProviderError> {
        let config = CompactifaiConfig::new(api_key);
        Self::new(config)
    }
...
impl LLMProvider for CompactifaiProvider {
    ...
    async fn transform_request(
        ...
        if let Some(temperature) = request.temperature {
            req["temperature"] = serde_json::to_value(temperature)
                .map_err(|e| ProviderError::serialization("compactifai", e.to_string()))?;
        }
    ...
    async fn chat_completion(
        ...
        let response = req_builder
            ...
            .send()
            .await
            .map_err(|e| ProviderError::network("compactifai", e.to_string()))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(match status {
                401 => ProviderError::authentication("compactifai", error_text),
                429 => ProviderError::rate_limit("compactifai", None),
                404 => ProviderError::model_not_found("compactifai", request.model),
                _ => ProviderError::api_error("compactifai", status, error_text),
            });
        }
    ...
    async fn chat_completion_stream(
        ...
        Err(ProviderError::not_implemented("compactifai", "Streaming not yet implemented"))
    }
    ...
    async fn calculate_cost(
        ...
        let model_info = self.supported_models.iter()
            .find(|m| m.id == model)
            .ok_or_else(|| ProviderError::model_not_found("compactifai", model.to_string()))?;
