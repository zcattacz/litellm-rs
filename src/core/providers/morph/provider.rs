...
const MORPH_CAPABILITIES: &[ProviderCapability] = &[
...
impl MorphProvider {
    pub async fn new(config: MorphConfig) -> Result<Self, ProviderError> {
        config
            .validate()
    ...
    async fn execute_request(
        ...
        let response = self
            ...
            .await
            .map_err(|e| ProviderError::network("morph", e.to_string()))?;
        ...
        if !status.is_success() {
            ...
            return Err(match status.as_u16() {
                400 => ProviderError::invalid_request("morph", error_body.to_string()),
                401 => ProviderError::authentication("morph", "Invalid API key"),
                404 => ProviderError::model_not_found("morph", "Model not found"),
                429 => ProviderError::rate_limit("morph", None),
                _ => HttpErrorMapper::map_status_code("morph", status.as_u16(), &error_body),
            });
        }

        serde_json::from_slice(&response_bytes).map_err(|e| {
            ProviderError::api_error("morph", 500, format!("Failed to parse response: {}", e))
        })
    }
}
...
impl LLMProvider for MorphProvider {
    ...
    async fn transform_request(
        ...
        serde_json::to_value(&request)
            .map_err(|e| ProviderError::invalid_request("morph", e.to_string()))
    }
    ...
    async fn chat_completion(
        ...
        debug!("Morph AI chat request: model={}", request.model);
    ...
    async fn calculate_cost(
        ...
        let model_info = get_model_info(model).ok_or_else(|| {
            ProviderError::model_not_found("morph", format!("Unknown model: {}", model))
        })?;
