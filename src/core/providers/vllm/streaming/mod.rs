...
pub type VLLMStreamInner = UnifiedSSEStream<
    Pin<Box<dyn Stream<Item = Result<Bytes, reqwest::Error>> + Send>>,
    OpenAICompatibleTransformer,
>;

/// Helper function to create vLLM stream
pub fn create_vllm_stream(
    stream: impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static,
) -> VLLMStreamInner {
    let transformer = OpenAICompatibleTransformer::new("vllm");
    UnifiedSSEStream::new(Box::pin(stream), transformer)
}

/// Wrapper stream that converts ProviderError to VLLMError for backward compatibility
pub struct VLLMStream {
    inner: VLLMStreamInner,
}

impl VLLMStream {
    pub fn new(stream: impl Stream<Item = Result<Bytes, reqwest::Error>> + Send + 'static) -> Self {
        Self {
            inner: create_vllm_stream(stream),
        }
    }
}

impl Stream for VLLMStream {
    type Item = Result<ChatChunk, VLLMError>;
...
) -> Result<Pin<Box<dyn Stream<Item = Result<ChatChunk, VLLMError>> + Send>>, VLLMError> {
