...
impl ProviderConfig for GigaChatConfig {
    fn validate(&self) -> Result<(), String> {
        if self.api_key.is_empty() {
            return Err("GigaChat API key (credentials) is required".to_string());
        }
...
impl LLMProvider for GigaChatProvider {
    ...
    async fn chat_completion(
        ...
        let headers = HeaderBuilder::new()
            ...
            .build_reqwest()
            .map_err(|e| ProviderError::invalid_request("gigachat", e.to_string()))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(ProviderError::api_error("gigachat", status, body));
        }
    ...
    async fn calculate_cost(
        ...
        let model_info = self
            ...
            .iter()
            .find(|m| m.id == model)
            .ok_or_else(|| ProviderError::model_not_found("gigachat", model.to_string()))?;
